...
interface SharePopupProps {
  ...
  url: string;
  title: string;
  onShare?: (platform: 'whatsapp' | 'instagram' | 'tiktok' | 'facebook' | 'copy_link') => void;
}
...
export function SharePopup({ isOpen, onClose, url, title, onShare }: SharePopupProps) {
            ...
            <div className="grid grid-cols-4 gap-3 mb-5">
              {socialPlatforms.map((p) => (
                <button
                  key={p.name}
                  onClick={() => {
                    window.open(p.url(url, fullTitle), '_blank', 'noopener,noreferrer');
                    if (onShare) onShare(p.platform as 'whatsapp' | 'instagram' | 'tiktok' | 'facebook' | 'copy_link');
                  }}
